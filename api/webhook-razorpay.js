...
// [FIX] This MUST be a top-level export, not inside the handler function —
// `export` statements are illegal anywhere except module top-level in JS.
// This tells Vercel not to auto-parse the request body, so we can read the
// exact raw bytes Razorpay sent — required for HMAC signature verification
// to match. Auto-parsed-then-reserialized JSON can differ byte-for-byte
// from the original (key order, whitespace), silently breaking every
// signature check.
export const config = {
  api: {
    bodyParser: false,
