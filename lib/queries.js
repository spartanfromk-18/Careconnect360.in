 ...
 *
 * findAvailableNurses:
 *   - Fetches active nurses whose speciality or experience overlaps the
 *     requested service or location keyword.
...
export async function findAvailableNurses(supabase, { location, service, date, time }) {
  // Step 1: Fetch all active nurses matching service/location.
  // We use ilike for a flexible keyword match on specialty and city fields.
  const locationKeyword = (location || '').split(',')[0].trim(); // Use city/first-segment

  const { data: candidates, error } = await supabase
    .from('nurses')
    .select('id, first_name, last_name, email, phone, speciality')
    .eq('status', 'active')
    .not('email', 'is', null)
    .or([
      service ? `speciality.ilike.%${service}%` : null,
      locationKeyword ? `experience.ilike.%${locationKeyword}%` : null,
    ].filter(Boolean).join(',') || 'status.eq.active') // fallback: all active nurses
