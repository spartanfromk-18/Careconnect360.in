...
/**
 * V2: Patient booking receipt.
 * Replaces sendBookingConfirmation for structured booking data.
 */
 ...
 * Contains only care type, area, date/time, and secure one-click action links.
 */
export async function sendNurseDispatchRequest({ nurseEmail, nurseName, bookingId, careType, service, location, scheduledDate, scheduledTime, acceptToken, declineToken }) {
  ...
  await deliver(() => sendEmail({
    to: nurseEmail,
    subject: `⚡ New Care Request Near You — ${esc(location || 'Your Area')}`,
    html: `
      <div style="${BASE_STYLE}">
        ...
        <div style="padding: 24px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
          ...
          <table style="width:100%; border-collapse:collapse; margin: 16px 0;">
            ...
            <tr><td style="padding:10px;border:1px solid #e5e7eb;"><strong>Area / Zone</strong></td><td style="padding:10px;border:1px solid #e5e7eb;">${esc(location || 'Nearby')}</td></tr>
...
/**
 * V2: Assignment confirmed — sent ONLY after atomic accept. Contains full patient PII.
 */
